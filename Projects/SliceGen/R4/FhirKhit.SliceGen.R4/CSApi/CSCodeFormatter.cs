...
namespace FhirKhit.SliceGen.CSApi
{
    public class CSCodeFormatter : ICodeFormatter
    {
        ...
        /// <returns></returns>
        public ElementNode SimpleFhirPath(CodeBlockNested block,
            ...
            String path)
        {
            ...
            block
                .OpenSummary()
                .AppendSummary($"Method to find elememt at simple fhir path '{path}'.")
                .CloseSummary()
                .AppendCode($"{methodModifiers} Element {methodName}(Element head)")
                .OpenBrace()
                ;
            ...
            while (i < pathItems.Length)
            {
                ...
                else
                {
                    if (node.TryGetChild(pathItem, out ElementNode next) == false)
                        return null;
                    node = next;
                    block.AppendCode($"head = head.GetNamedChildren({node.Name});");
                    block.AppendCode($"if (head == null.{node.Name};");
                }
            }
            return node;
        }
